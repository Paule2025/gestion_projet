...
interface ProjectProps {
    ...
    admin: number;
    style : boolean;
    onDelete?: (projectId: string) => void; // Optional delete function prop
...
const ProjectComponent: FC<ProjectProps> = ({ project, admin, style,onDelete}) => {
    ...
    const handleDeleteClick = () => {
        const isCorfirmed = window.confirm("Êtes-vous sûr de vouloir supprimer ce projet ?"); //demande de confirmation avant de supprimer le projet
        if (isCorfirmed && onDelete) {
            onDelete(project.id); //appelle la fonction de suppression
        }
    }
    ...
    return (
        <div key={project.id} className={`${style ? 'border border-base-300 p-5 shadow-sm ' : ''}text-base-content rounded-xl w-full text-left`}>
            ...
            <div className='flex flex-col mb-3'>
                ...
                <progress className="progress progress-warning w-full" value={inProgressPercentage} max="100">
                </progress>
                <div className='flex'>
                    <span className={`text-gray-400 mt-2 ${textSizeClass}`}>
            ...
            <div className='flex flex-col mb-3'>
                ...
                <progress className="progress progress-success w-full" value={progressPercentage} max="100">
                </progress>
                <div className='flex'>
                    <span className={`text-gray-400 mt-2 ${textSizeClass}`}>
                        {progressPercentage}%
            ...
            <div className='flex'>
                ...
                {admin === 1 && (
                    <button className='btn btn-sm ml-3 hover:btn-error' onClick={handleDeleteClick}>
                        <Trash className='w-4' />
                    </button>
                )}
            </div>
