...
const Navbar = () => {
    ...
    const renderLinks = (className: string) =>
        navLinks.map(({ href, label }) => {
            return <Link href={href} key={href} className={`btn-sm ${className} ${isActiveLink(href) ? "btn-primary" : ""} `}>
                {label}
            </Link>
    ...
    return (
        <div className='border-b-2 border-base-300 px-5 md:px-[10%] py-4 relative'>
